    ...
    <header className="relative w-full bg-white text-gray-900 z-50 font-sans border-b border-gray-200">
      ...
      <div className="max-w-[1400px] mx-auto px-4 sm:px-6 py-3.5 flex items-center justify-between gap-4 relative z-50">
        ...
        <div className="flex items-center gap-2.5 sm:gap-3.5 text-[13px] font-medium flex-shrink-0">
          ...
          {/* Article Notifications Bell Icon */}
          <div className="relative block z-[100]" ref={notifDropdownRef}>
            <button
              type="button"
              onClick={() => setIsNotificationsOpen((prev) => !prev)}
              className="relative p-2 text-gray-700 hover:text-[#BF1E2D] hover:bg-gray-100 rounded-full transition-colors cursor-pointer"
              title="Article Notifications"
              aria-label="Article Notifications"
            >
              <Bell size={19} />
              {articleNotifications.filter((a) => a.status === "Pending review").length > 0 && (
                <span className="absolute top-0.5 right-0.5 min-w-[17px] h-[17px] px-1 bg-[#BF1E2D] text-white text-[9.5px] font-black rounded-full flex items-center justify-center border-2 border-white shadow-2xs animate-pulse">
                  {articleNotifications.filter((a) => a.status === "Pending review").length}
                </span>
              )}
            </button>

            {/* ARTICLE NOTIFICATION POPOVER */}
            {isNotificationsOpen && (
              <div className="absolute right-0 top-full mt-2 w-80 bg-white border border-gray-200 shadow-2xl rounded-xl text-left z-[1000] overflow-hidden font-sans animate-in fade-in slide-in-from-top-2 duration-150">
                <div className="px-4 py-3 bg-gray-900 text-white flex items-center justify-between border-b border-gray-800">
                  <div className="flex items-center gap-2 font-bold text-xs">
                    <Bell size={14} className="text-[#BF1E2D]" />
                    <span>ARTICLE NOTIFICATIONS</span>
                  </div>
                  <span className="text-[10px] bg-[#BF1E2D] text-white font-bold px-2 py-0.5 rounded-full">
                    {articleNotifications.length} ARTICLES
                  </span>
                </div>
                ...
                <div className="max-h-80 overflow-y-auto divide-y divide-gray-100">
                  {articleNotifications.length === 0 ? (
                    <div className="p-6 text-center">
                      <p className="text-xs font-bold text-gray-700 mb-1">No articles found</p>
                      <p className="text-[11px] text-gray-400">There are currently no articles submitted or pending review.</p>
                    </div>
                  ) : (
                    articleNotifications.map((article) => (
                      <div key={article.id} className="p-3 hover:bg-gray-50 transition-colors flex items-start gap-2.5">
                        <span className={`w-2 h-2 rounded-full mt-1.5 shrink-0 ${article.status === "Pending review" ? "bg-amber-500 animate-ping" : article.status === "Published" ? "bg-emerald-500" : "bg-gray-400"}`} />
                        <div className="min-w-0 flex-1">
                          <p className="text-xs font-bold text-gray-900 line-clamp-2 leading-snug">
                            {article.title}
                          </p>
                          <div className="flex items-center justify-between mt-1 text-[10px]">
                            <span className={`font-bold px-1.5 py-0.2 rounded ${article.status === "Pending review" ? "bg-amber-100 text-amber-800" : article.status === "Published" ? "bg-emerald-100 text-emerald-800" : "bg-gray-100 text-gray-600"}`}>
                              {article.status.toUpperCase()}
                            </span>
                            <span className="text-gray-400">{article.date || "Recent"}</span>
                          </div>
                        </div>
                      </div>
                    ))
                ...
                <div className="p-2.5 bg-gray-50 border-t border-gray-200 text-center">
                  <Link
                    href={currentUser?.role === "Admin" ? "/admin" : "/writer"}
                    onClick={() => setIsNotificationsOpen(false)}
                    className="text-[11px] font-bold text-[#BF1E2D] hover:underline"
                  >
                    View All Articles in Workspace →
                  </Link>
                </div>
              </div>
            )}
